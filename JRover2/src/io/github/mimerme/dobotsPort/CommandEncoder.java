...
public class CommandEncoder {
    ...
    public static ByteArrayBuffer parseCommand(Rover2Controller controller, ByteArrayBuffer arrayBuffer) throws IOException {
    	...
    	System.out.println("memes command");

        try {
            CommandEncoder commandEncoder = new CommandEncoder();
            commandEncoder.getClass();
            Message response = new Message(commandEncoder, buffer);
            arrayBuffer.clear();
            arrayBuffer.append(buffer, response.size(), buffer.length - response.size());
        	System.out.println("memes op " + response.getOpCode());

            switch (response.getOpCode()) {
                case VIDEO_DATA /*1*/:
                	System.out.println("memes command");
                    parseLoginResp(controller, response);
                    break;
                case VERIFY_RESP /*3*/:
                    parseVerifyResp(controller, response);
                    break;
                case VIDEO_START_RESP /*5*/:
                    parseVideoStartResp(controller, response);
                    break;
                case BATTERY_POWER_RESP /*252*/:
                    parseBatteryPowerResp(controller, response);
                    break;
            }
    ...
    public static void parseBatteryPowerResp(Rover2Controller controller, Message response) throws IOException {
        int raw_battery = response.getContent().get(VIDEO_END_LEN);
        double battery = (100.0d * ((double) raw_battery)) / 7.0d;
        controller.setBatteryPower(battery);
        String str = TAG;
        Object[] objArr = new Object[VERIFY_REQ];
        objArr[VIDEO_END_LEN] = Integer.valueOf(raw_battery);
        objArr[VIDEO_DATA] = Double.valueOf(battery);
        System.out.println(String.format("battery value: %d (raw), %.3f (%%)"));
    }

    public static boolean parseLoginResp(Rover2Controller controller, Message response) {
    	System.out.println("memes");
        try {
            System.out.println( "cmdLoginResp");
            ByteBuffer content = response.getContent();
            if (content.getShort(VIDEO_END_LEN) != (short) 0) {
                return false;
            }
            int i;
            byte[] cameraBuffer = new byte[VIDEO_DATA_IDX];
            content.position(VERIFY_REQ);
            content.get(cameraBuffer);
            String cameraId = new String(cameraBuffer).trim();
            controller.setCameraId(cameraId);
            System.out.println( "loginResp(cameraId):" + cameraId);
            int[] device = new int[VIDEO_START_REQ_LEN];
            for (i = VIDEO_END_LEN; i < VIDEO_START_REQ_LEN; i += VIDEO_DATA) {
                device[i] = content.get(i + HEAD_LEN);
            }
            String deviceId = device[VIDEO_END_LEN] + "." + device[VIDEO_DATA] + "." + device[VERIFY_REQ] + "." + device[VERIFY_RESP];
            controller.setDeviceId(deviceId);
            System.out.println( "loginResp(deviceId):" + deviceId);
            content.order(ByteOrder.LITTLE_ENDIAN);
            int[] challenge_verify = new int[VIDEO_START_REQ_LEN];
            for (i = VIDEO_END_LEN; i < VIDEO_START_REQ_LEN; i += VIDEO_DATA) {
                challenge_verify[i] = content.getInt((i * VIDEO_START_REQ_LEN) + CHALLENGE_VERIFY_IDX);
            }
            content.order(ByteOrder.LITTLE_ENDIAN);
            int[] reverse_challenge = new int[VIDEO_START_REQ_LEN];
            for (i = VIDEO_END_LEN; i < VIDEO_START_REQ_LEN; i += VIDEO_DATA) {
                reverse_challenge[i] = content.getInt((i * VIDEO_START_REQ_LEN) + REVERSE_CHALLENGE_IDX);
            }
            controller.setReverseChallenge(reverse_challenge);
            int[] challenge = controller.getChallenge();
            int[] arr1 = new int[VIDEO_DATA];
            arr1[VIDEO_END_LEN] = challenge[VIDEO_END_LEN];
            int[] arr2 = new int[VIDEO_DATA];
            arr2[VIDEO_END_LEN] = challenge[VIDEO_DATA];
            int[] arr3 = new int[VIDEO_DATA];
            arr3[VIDEO_END_LEN] = challenge[VERIFY_REQ];
            int[] arr4 = new int[VIDEO_DATA];
            arr4[VIDEO_END_LEN] = challenge[VERIFY_RESP];
            String challenge_1 = Utils.intToHex(challenge[VIDEO_END_LEN]);
            String challenge_2 = Utils.intToHex(challenge[VIDEO_DATA]);
            String challenge_3 = Utils.intToHex(challenge[VERIFY_REQ]);
            String challenge_4 = Utils.intToHex(challenge[VERIFY_RESP]);
            System.out.println("challenge:" + challenge_1 + "," + challenge_2 + "," + challenge_3 + "," + challenge_4);
            BlowFish blowfish = new BlowFish(f18P, f19S);
            blowfish.initialize(controller.getKey().getBytes(), controller.getKey().length());
            blowfish.encipher(arr1, arr2);
            blowfish.encipher(arr3, arr4);
            challenge_1 = Utils.intToHex(arr1[VIDEO_END_LEN]);
            challenge_2 = Utils.intToHex(arr2[VIDEO_END_LEN]);
            challenge_3 = Utils.intToHex(arr3[VIDEO_END_LEN]);
            challenge_4 = Utils.intToHex(arr4[VIDEO_END_LEN]);
            String verify_1 = Utils.intToHex(challenge_verify[VIDEO_END_LEN]);
            String verify_2 = Utils.intToHex(challenge_verify[VIDEO_DATA]);
            String verify_3 = Utils.intToHex(challenge_verify[VERIFY_REQ]);
            String verify_4 = Utils.intToHex(challenge_verify[VERIFY_RESP]);
            System.out.println( "===============================");
            System.out.println( "loginResp(challenge):" + challenge_1 + "," + challenge_2 + "," + challenge_3 + "," + challenge_4);
            System.out.println( "loginResp(verification ):" + verify_1 + "," + verify_2 + "," + verify_3 + "," + verify_4);
            System.out.println( "===============================");
    ...
    public static void parseVerifyResp(Rover2Controller controller, Message response) {
        System.out.println( "cmdVerifyResp");
        try {
    ...
    public static void parseVideoStartResp(Rover2Controller controller, Message response) throws IOException {
        int nCameraId = response.getContent().getInt(VERIFY_REQ);
        System.out.println( "cmdVideoStartResp:" + nCameraId);
        controller.connectMediaReceiver(nCameraId);
    }
    ...
    public static byte[] cmdKeepAlive() throws IOException {
        ...
        CommandEncoder commandEncoder = new CommandEncoder();
        commandEncoder.getClass();
        return new Command(KEEP_ALIVE, VIDEO_END_LEN).output();
    }

    public static byte[] cmdBatteryPowerReq() throws IOException {
        System.out.println( "batteryPowerReq");
        CommandEncoder commandEncoder = new CommandEncoder();
        commandEncoder.getClass();
        return new Command(BATTERY_POWER_REQ, VIDEO_END_LEN).output();
    }

    public static byte[] cmdDecoderControlReq(int value) throws IOException {
        System.out.println( "decoderControlReq");
        CommandEncoder commandEncoder = new CommandEncoder();
        commandEncoder.getClass();
        Command cmd = new Command(DECODER_CONTROL_REQ, VIDEO_DATA);
        cmd.getContent().put((byte) value);
        return cmd.output();
    }

    public static byte[] cmdDeviceControlReq(int value1, int value2) throws IOException {
        System.out.println( "deviceControlReq");
        CommandEncoder commandEncoder = new CommandEncoder();
        commandEncoder.getClass();
        Command cmd = new Command(DEVICE_CONTROL_REQ, VERIFY_REQ);
        cmd.getContent().put((byte) value1);
        cmd.getContent().put((byte) value2);
        return cmd.output();
    }
    ...
    public static byte[] cmdLoginReq(int[] challenge) throws IOException {
        ...
        CommandEncoder commandEncoder = new CommandEncoder();
        commandEncoder.getClass();
        Command cmd = new Command(VIDEO_END_LEN, VERIFY_REQ_LEN);
        ByteBuffer content = cmd.getContent();
        for (int i = VIDEO_END_LEN; i < challenge.length; i += VIDEO_DATA) {
            content.putInt(challenge[i]);
        }
        return cmd.output();
    }

    public static byte[] cmdVerifyReq(Rover2Controller controller, String paramString, int[] reverse_challenge) throws IOException {
        System.out.println( "cmdVerifyReq");
        int[] arrayOfInt1 = new int[VIDEO_DATA];
        arrayOfInt1[VIDEO_END_LEN] = reverse_challenge[VIDEO_END_LEN];
        int[] arrayOfInt2 = new int[VIDEO_DATA];
        arrayOfInt2[VIDEO_END_LEN] = reverse_challenge[VIDEO_DATA];
        int[] arrayOfInt3 = new int[VIDEO_DATA];
        arrayOfInt3[VIDEO_END_LEN] = reverse_challenge[VERIFY_REQ];
        int[] arrayOfInt4 = new int[VIDEO_DATA];
        arrayOfInt4[VIDEO_END_LEN] = reverse_challenge[VERIFY_RESP];
        BlowFish localBlowFish = new BlowFish(f18P, f19S);
        localBlowFish.initialize(controller.getKey().getBytes(), controller.getKey().length());
        localBlowFish.encipher(arrayOfInt1, arrayOfInt2);
        localBlowFish.encipher(arrayOfInt3, arrayOfInt4);
        CommandEncoder commandEncoder = new CommandEncoder();
        commandEncoder.getClass();
        Command cmd = new Command(VERIFY_REQ, VERIFY_REQ_LEN);
        ByteBuffer content = cmd.getContent();
        content.putInt(arrayOfInt1[VIDEO_END_LEN]);
        content.putInt(arrayOfInt2[VIDEO_END_LEN]);
        content.putInt(arrayOfInt3[VIDEO_END_LEN]);
        content.putInt(arrayOfInt4[VIDEO_END_LEN]);
        return cmd.output();
    }

    //TODO
    public static byte[] cmdMediaLoginReq(int nID) throws IOException {
 /*       CommandEncoder commandEncoder = new CommandEncoder();
        commandEncoder.getClass();
        Media media = new Media(VIDEO_END_LEN, VIDEO_START_REQ_LEN);
        media.getContent().putInt(nID); media.output();*/
        return null;
    }

    public static ByteArrayBuffer parseMediaCommand(Rover2Controller controller, ByteArrayBuffer arrayBuffer) throws IOException {
        byte[] buffer = arrayBuffer.toByteArray();
        try {
            CommandEncoder commandEncoder = new CommandEncoder();
            commandEncoder.getClass();
            ...
            switch (response.getOpCode()) {
                case VIDEO_DATA /*1*/:
                    parseVideoData(controller, response);
                    break;
                case VERIFY_REQ /*2*/:
                    break;
                default:
    ...
    public static void parseVideoData(Rover2Controller controller, Message response) {
        ByteBuffer content = response.getContent();
        byte[] data = new byte[content.getInt(VIDEO_DATA_LEN_IDX)];
    ...
    public static byte[] cmdVideoEnd() throws IOException {
        CommandEncoder commandEncoder = new CommandEncoder();
        commandEncoder.getClass();
        return new Command(VIDEO_END, VIDEO_END_LEN).output();
    }

    public static byte[] cmdVideoStartReq() throws IOException {
        CommandEncoder commandEncoder = new CommandEncoder();
        commandEncoder.getClass();
        Command cmd = new Command(VIDEO_START_REQ_LEN, VIDEO_START_REQ_LEN);
        cmd.getContent().put((byte) 1);
        return cmd.output();
    }
}
